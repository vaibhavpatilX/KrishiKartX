...
if(isset($_SESSION['farmer'])){
  ...
  $ProductCount=0;
  $CustomerCount=0;
  $result = $conn->query("SELECT * FROM `order` WHERE FarmerId='$FarmerId' AND OrderStatus='Pending'"); 
  if($result){
	  ...
	  $result = $conn->query("SELECT * FROM `order` WHERE FarmerId='$FarmerId' AND OrderStatus='Delivered'"); 
	  if($result){
		  ...
		  $result = $conn->query("SELECT * FROM `order` WHERE FarmerId='$FarmerId' AND OrderStatus='Canceled'"); 
		  if($result){
	 ...
	 $result = $conn->query("SELECT * FROM `order` WHERE FarmerId='$FarmerId'"); 
		  if($result){
	...
	$result = $conn->query("SELECT * FROM product WHERE FarmerId='$FarmerId'"); 
			  if($result){
 ...
 $result = $conn->query("SELECT * FROM customer"); 
				  if($result){
					  ...
					  while($row = $result->fetch_assoc()){
						  ...
						  $result1 = $conn->query("SELECT * FROM `order` WHERE CustomerId='$CustomerId' AND FarmerId='$FarmerId'"); 
				  if($result1){
...
<div class="content">
			<section class="info-boxes">
				...
				<div class="info-box">
					...
					<div class="box-content">
						<span class="big"><?php echo $ProductCount; ?></span>
						Total Products 
					</div>
				</div>
